...
- required_suspect_ids: is_critical: true の場合は必ず設定すること。description に名前が登場する容疑者のidを suspects 配列から正確に選んで1〜2個設定する。この設定により、証拠だけでなく対象容疑者への聞き込み（プロフィール閲覧＋全証言聴取）も発火条件となり、証拠だけで犯人が特定されるゲーム上の不自然さを防ぐ。is_critical: false の場合は省略してよい
