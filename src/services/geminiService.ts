...
- 各容疑者にdefault_wrong_pursuit_responseを設定すること（見当違いの発言で的外れな追及をされたときの容疑者ごとの反応。性格に合った1文。「証言」「証拠」「矛盾」「推理」などのゲーム用語を使わないこと）
...
証拠クロス参照システム。単体では意味が薄い証拠が2〜3個組み合わさって初めて「真相の断片」が解放される仕組み。
...
- これはプレイヤーが**誤った発言**を選択したときに表示される容疑者のリアクション（1文）
- キャラクターの性格を反映した自然な台詞にすること。「証言」「証拠」「矛盾」「推理」などのゲーム用語を使わないこと
- 例：「……（首を振る）その話と、これとがどう繋がるというんだ。」
...
- wrong_link_rebuttal: 提示された物と推理が噛み合わない時の反論（1〜2文）。論理の穴を突いて余裕を見せる。キャラクターの性格を反映した自然な台詞にし、「証言」「証拠」「矛盾」「推理」などのゲーム用語を使わないこと
