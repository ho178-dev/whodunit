...
ファイルが存在しない場合は `default.png` 自動表示されるため、未配置でも動作する。
...
- キャラクターイラスト用Model
  DreamShaper XL（商用利用確認）
  cetusMix_Whalefall2（商用利用確認）
...
| `default.png`           | デフォルト画像(中性的)             |

- **フォーマット**: PNG（推奨サイズ: 256×320px 以上、縦長）
- **フォールバック**: 画像なしの場合 `default.png` を表示
