...
ゲームロジックとストア状態遷移を検証する。`npm run test:run` で実行（CI組み込み済み）。

| ファイル                            | 内容                                                                                               |
| ----------------------------------- | -------------------------------------------------------------------------------------------------- |
| `src/test/gameFlow.test.ts`         | 固定3シナリオ × 完勝/誤謬 の6パターン + エッジケース（AP枯渇・セーブロード・null accusation_data） |
| `src/test/fixedScenarios.test.ts`   | 固定シナリオ3本の論理整合性チェック（validateScenario）                                            |
| `src/test/scenarioRegistry.test.ts` | シナリオレジストリユーティリティのテスト                                                           |
| `src/test/saveLoad.test.ts`         | セーブ/ロード機能の正常系・異常系テスト                                                            |
...
UIレイヤーの動作確認（ブラウザ上での実際のレンダリングとクリック操作）。Vite 開発サーバーを自動起動して実行する。

| ファイル               | 内容                                                                   |
| ---------------------- | ---------------------------------------------------------------------- |
| `e2e/gameFlow.spec.ts` | タイトル画面スモークテスト・完勝エンド・誤謬エンドのUIレンダリング確認 |
...
npx playwright test --ui         # UIモード（ブラウザで視覚確認）
npx playwright test --headed     # ブラウザを表示して実行
```

> **CI での動作**: PR 作成時に GitHub Actions が Vitest（ユニット）・Playwright（E2E）を自動実行する。
